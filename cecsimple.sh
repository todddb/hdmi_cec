...
CECCLIENT="/usr/bin/cec-client -s -d 1 -o ToddsPi -f $CECLOG"
...
case "${1}" in
    ...
    activesrc)
        ...
        send_command "tx 1F:82:11:00"
    ;;

    hdmi1)
	send_command "tx 1F:82:10:00"
    ;;
    
    hdmi2)
	send_command "tx 1F:82:20:00"
    ;;

    hdmi3)
	send_command "tx 1F:82:30:00"
    ;;
	
    mute)
        # Me to TV -> user pressed mute
		send_command "tx 10:44:43"
	;;
	
    volup)
        # Me to TV -> user pressed vol up
		send_command "tx 10:44:41"
	;;
   
	voldown)
        # Me to TV -> user pressed vol down
		send_command "tx 10:44:42"
	;;

    *)
        echo $"Usage: $0 {tvon|tvoff|allon|alloff|activesrc|hdmi1|hdmi2|hdmi2|mute|volup|voldown}"
        exit 1
    ;;
esac
